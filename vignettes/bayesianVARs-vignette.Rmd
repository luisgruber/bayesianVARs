...
\end{equation}
Both $\bm Q_t = \mathrm{diag} (e^{h_{1t}}, \dots, e^{h_{Mt}})$ and $\bm V_t = \mathrm{diag} (e^{h_{M+1,t}}, \dots, e^{h_{M+r,t}})$ are diagonal matrices of dimension $M$ and $r$, respectively, and $\bm \Lambda$ is the $M \times r$ matrix of factor loadings. This is obviously equivalent to introducing $r$ conditionally independent latent factors $\bm f_t \sim \mathcal N_r(\bm 0, \bm V_t)$ and rewriting the error term in \eqref{eq:var} as 
\begin{equation}
\bm \varepsilon_t^\prime = \bm f_t^\prime \bm \Lambda^\prime + \bm \eta_t^\prime,    
\end{equation}
...
phi <- coef(mod)
posterior_heatmap(phi, median, detect_lags = TRUE, 
                  border_color = rgb(0,0,0, alpha = 0.2))
posterior_heatmap(phi, IQR, detect_lags = TRUE, 
                  border_color = rgb(0,0,0, alpha = 0.2))
```
